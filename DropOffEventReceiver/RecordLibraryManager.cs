...
namespace DropOffEventReceiver
{
    ...
    public class RecordLibraryManager : IDisposable, ITraceable
    {
        #region fields
        ...
        {
            UnifiedLoggingServer.LogMedium(header + "DoLurl_:" + this.DropOffLibUrl);
            UnifiedLoggingServer.LogMedium(header + "DoLtit_:" + this.DropOffLibTitle);
            UnifiedLoggingServer.LogMedium(header + "DoLsnd_:" + this.DropOffLibrary.SendToLocationUrl);
            foreach (SPList enabledLibrary in this.EnabledLibraries)
        ...
        private List<SPList> ScreenLibraries(SPWeb spWeb)
        {
            ...
            foreach (SPList librarySpList in spWeb.Lists)
            {
                if ((librarySpList is SPPictureLibrary || librarySpList is SPDocumentLibrary)
                    ...
                    && !librarySpList.Title.Contains("Converted Forms")
                    && !librarySpList.Title.Contains("Customized Reports")
                    && !librarySpList.Title.Contains("Form Templates")
                    && !librarySpList.Title.Contains("Hold Reports")
                    && !librarySpList.Title.Contains("Master Page Gallery")
                    && !librarySpList.Title.Contains("Record Library")
                    && !librarySpList.Title.Contains("Reporting Templates")
                    && !librarySpList.Title.Contains("Style Library")
                    && !librarySpList.Title.Contains("Theme Gallery")
                    && !librarySpList.Title.Contains("wfpub"))
                {
                    try
                    {
        ...
        /// <param name="libraryDesc">The library Desc.</param>
        /// <returns>
        /// The System.Boolean.
        /// </returns>
